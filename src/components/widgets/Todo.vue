...
<div class="card">
    <div class="card-body">
        <b-dropdown class="float-end" toggle-class="card-drop p-0" variant="black" menu-class="dropdown-menu-end">
        ...
        <div class="todoapp">
            <div class="row align-items-center">
                ...
                <div class="col-sm-6">
                    <a href="javascript: void(0);" class="float-end btn btn-light btn-xs" @click="archiveTodo">Archive</a>
                </div>
            </div>
            <simplebar style="max-height: 292px;">
                <div>
                    <ul id="todo-list" class="list-group list-group-flush todo-list" style="max-height: 320px">
                        <li class="list-group-item border-0 ps-0" v-for="(todo, index) in todoData" :key="index">
                            <div class="form-check mb-0"><input  type="checkbox" :id="`${todo.id}`" :checked="`${todo.done ? true : ''}`" @change="updateCheck(index)" class="form-check-input todo-done">
                                <label class="form-check-label" :for="`${todo.id}`"> <s v-if="todo.done === true">{{ todo.todo }}</s>
