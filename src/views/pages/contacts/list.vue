...
export default {
  ...
  computed: {
    contactsData() {
      return this.contacts.filter((record) => {
        return (
          record.name.toLowerCase().includes(this.search.toLowerCase()) ||
          record.designation.toLowerCase().includes(this.search.toLowerCase())
        );
      });
    },
  },
    ...
    <div class="row mb-2">
      <div class="col-sm-4">
        <a href="javascript:void(0);" class="btn btn-danger mb-2"
          ><i class="mdi mdi-plus-circle me-2"></i> Add New</a
        >
      </div>
      <div class="col-sm-8">
        <div class="float-sm-end">
          <form
            class="d-flex align-items-start flex-wrap justify-content-sm-end"
          >
            <div class="d-flex align-items-start flex-wrap me-2">
              <label for="membersearch-input" class="visually-hidden"
                >Search</label
              >
              <input
                v-model="search"
